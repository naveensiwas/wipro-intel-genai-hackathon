...
"""
from app.core.logging import get_logger, log_section, log_success, log_error, log_step
...
try:
    from app.rag.data_loader import (
        load_symptom_documents,
        load_condition_documents,
        load_preventive_documents,
    )
    ...
    log_success(
        logger,
        f"Documents loaded - Symptoms: {len(s)}, Conditions: {len(c)}, Preventive: {len(p)}, Total: {total}",
    )
...
try:
    ...
    _ = (build_rag_chain, retrieve_sources)
...
try:
    ...
    _ = (render_sidebar, init_chat_state, render_chat_history, add_message)
