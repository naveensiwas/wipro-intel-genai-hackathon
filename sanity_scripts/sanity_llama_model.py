...
try:
    ...
    # Timing the inference call to measure latency
    start_time = time.time()
    response = llm.invoke(test_prompt)
    end_time = time.time()

    # Calculating elapsed time and logging response details
    elapsed_seconds = end_time - start_time
    content = response.content if hasattr(response, "content") else str(response)

    #
    log_msg = (f"Inference completed successfully in {elapsed_seconds:.2f} seconds. "
               f"Response length: {len(content)} characters.")
    log_success(logger, log_msg)

    # Logging the prompt and response content for debugging (truncated if too long)
